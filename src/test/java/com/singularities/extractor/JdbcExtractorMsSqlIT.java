...
class JdbcExtractorMsSqlIT extends JdbcTest {
  ...
  void extractData() throws SQLException {
    ...
    JdbcQuery pJdbcQuery =
        JdbcQuery.newBuilder()
            .setHost(getHost())
            .setPort(getPort())
            ...
            .setFetchSize(1000L)
            .build();
    String pParquetUrl = String.format("file://%s/Person.parquet", Files.createTempDir().getPath());
    // Expected
    final Properties eConnectionProperties = pJdbcQuery.getConnectionProperties();
    ...
    // Call
    final Dataset<Row> aParquet = extractor.extractIntoDataset(pJdbcQuery);
    aParquet.write().parquet(pParquetUrl);
    // Assertions
    verify(mDataFrameReader, atLeastOnce())
        .jdbc(
            pJdbcQuery.getConnectionUrl(),
            pJdbcQuery.getTable(),
            pJdbcQuery.getColumnName(),
            pJdbcQuery.getLowerBound(),
            pJdbcQuery.getUpperBound(),
            pJdbcQuery.getNumPartitions(),
            eConnectionProperties);
    assertEquals(eTotalRows, aParquet.count());
    assertArrayEquals(eColumns, aParquet.columns());
    assertEquals(
        eRow, aParquet.select("FirstName", "LastName", "Age").toJavaRDD().first().mkString(","));
  }
}
